  ...
  async viewByDepartment() {
    ...
    return new Promise(resolve => {
      ...
      db.query(department, (err, rows) => {
        ...
        departmentList = JSON.parse(JSON.stringify(rows));
      ...
      return departmentList.findIndex(department => department.name === selection.selectDepartment);
    }).then(index => {
      console.log(index);
      ...
      WHERE department.id = ${departmentList[index].id};
      `;
      db.query(sql, (err, rows) => {
        if (err) {
          console.log(err);
